BACKBONE <- function(data, lib_path = NULL, safer_check = TRUE){ 
    #### package name
    ...
    if(safer_check == TRUE){
        saferDev:::.base_op_check(
            ...
            external.package_name = package_name
        )
    }
    ...
    if( ! base::is.null(path_out)){ # for all arguments that can be NULL, write like this:
    ...
    if( ! (base::all(base::class(x = arg_user_setting) %in% base::c("list", "NULL"), na.rm = TRUE) & base::length(x = arg_user_setting) == 0)){
